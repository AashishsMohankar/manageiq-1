...
module EvmDba
  ...
  def self.with_options(*option_types, &block)
    ...
    Optimist.options(EvmRakeHelper.extract_command_options) do
      option_types.each do |type|
        ...
        when :aliyun
          opt :aliyun_region,         "Destination depot AWS region", :type => :string
  ...
  CONNECT_OPT_KEYS = %i(uri uri_username uri_password aliyun_region remote_file_name skip_directory).freeze
...
namespace :evm do
  namespace :db do
    ...
    namespace :backup do
      ...
      task :remote do
        opts = EvmDba.with_options(:remote_uri, :aliyun, :remote_file, :splitable, :db_credentials)
    ...
    namespace :dump do
      ...
      task :remote do
        opts = EvmDba.with_options(:remote_uri, :aliyun, :remote_file, :splitable, :db_credentials, :exclude_table_data)
    ...
    namespace :restore do
      ...
      task :remote => :environment do
        opts = EvmDba.with_options(:remote_uri, :aliyun, :db_credentials)
