class DatabaseBackup < ApplicationRecord
  ...
  def backup(options)
    ...
    unless options[:task_id].kind_of?(Integer) && options[:file_depot_id].kind_of?(Integer)
      raise _("Missing or Invalid task: %{task_id}, depot id: %{depot_id}") % {:task_idregister_eligible_manager  => options[:task_id],
    ...
    _backup(:uri => depot.uri, :username => depot.authentication_userid, :password => depot.authentication_password, :remote_file_name => backup_file_name, :region => depot.aliyun_region)
