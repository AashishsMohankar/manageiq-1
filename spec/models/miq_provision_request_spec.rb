describe MiqProvisionRequest do
  context "#request_task_class_from" do
    it "retrieves the provision class when the vm has EMS" do
      ...
      ems = FactoryBot.create(:ems_alibaba)
      vm = FactoryBot.create(:vm_alibaba, :ext_management_system => ems)
      expect(described_class.request_task_class_from('options' => {:src_vm_id => vm.id})).to eq ManageIQ::Providers::Alibaba::CloudManager::Provision
    end
