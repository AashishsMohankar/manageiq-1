describe ServiceOrchestration do
  let(:manager_by_setter)  { FactoryBot.create(:ems_alibaba) }
  let(:template_by_setter) { FactoryBot.create(:orchestration_template) }
  let(:manager_by_dialog)  { FactoryBot.create(:ems_alibaba) }
  let(:template_by_dialog) { FactoryBot.create(:orchestration_template) }
  let(:manager_in_st)      { FactoryBot.create(:ems_alibaba) }
  let(:template_in_st)     { FactoryBot.create(:orchestration_template) }
  let(:deployed_stack)     { FactoryBot.create(:orchestration_stack_alibaba) }
  ...
  describe "#my_zone" do
    ...
    it "service, takes the zone from orchestration_manager" do
      ems = FactoryBot.create(:ems_alibaba, :zone => FactoryBot.create(:zone))
      deployed_stack.direct_vms << FactoryBot.create(:vm_alibaba, :ext_management_system => ems)
  ...
  describe "#stack_options" do
    before do
      allow_any_instance_of(ManageIQ::Providers::Alibaba::CloudManager::OrchestrationServiceOptionConverter).to(
  ...
  describe '#deploy_orchestration_stack' do
    it 'creates a stack through cloud manager' do
      allow(ManageIQ::Providers::Alibaba::CloudManager::OrchestrationStack).to receive(:raw_create_stack) do |manager, name, template, opts|
    ...
    it 'always saves options even when the manager fails to create a stack' do
      ...
      allow_any_instance_of(ManageIQ::Providers::Alibaba::CloudManager).to receive(:stack_create).and_raise(provision_error, 'test failure')
  ...
  describe '#orchestration_stack_status' do
    ...
    it 'returns current stack status through provider' do
      status_obj = ManageIQ::Providers::Alibaba::CloudManager::OrchestrationStack::Status.new('CREATE_COMPLETE', 'no error')
  ...
  describe '#all_vms' do
    it 'returns all vms from a deployed stack' do
      vm1 = FactoryBot.create(:vm_alibaba)
      vm2 = FactoryBot.create(:vm_alibaba)

      child_stack = FactoryBot.create(:orchestration_stack_alibaba, :parent => deployed_stack)
  ...
  describe '#post_provision_configure' do
    before do
      allow(ManageIQ::Providers::Alibaba::CloudManager::OrchestrationStack)
    ...
    it 'sets owners for all vms included in the stack' do
      vms = [FactoryBot.create(:vm_alibaba), FactoryBot.create(:vm_alibaba)]
