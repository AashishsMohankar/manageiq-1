describe ServiceTemplate do
  ...
  describe '.class_from_request_data' do
    ...
    it 'returns the correct non generic type' do
      template_class = ServiceTemplate.class_from_request_data('prov_type' => 'alibaba')
  ...
  let(:ems) { FactoryBot.create(:ems_alibaba) }
  let(:vm) { FactoryBot.create(:vm_alibaba, :ext_management_system => ems) }
  let(:flavor) { FactoryBot.create(:flavor_alibaba) }
  ...
  describe '#update_catalog_item' do
    let(:new_vm) { FactoryBot.create(:vm_alibaba, :ext_management_system => ems) }
  ...
  context "catalog_item_types" do
    it "only returns generic with no providers" do
      expect(ServiceTemplate.catalog_item_types).to match(
        hash_including('alibaba'  => {:description => 'Alibaba',  :display => false},
    ...
    it "returns orchestration template and generic" do
      ...
      expect(ServiceTemplate.catalog_item_types).to match(
        hash_including('alibaba'                => { :description => 'Alibaba',
                                                    :display     => false },
