...
describe ManageIQ::Providers::Inventory::Persister do
  ...
  it "tests we can serialize inventory object with nested lazy references" do
    ...
    expect(vm.hardware.guest_os_full_name).to eq("alibaba")
  ...
  def populate_test_data(persister)
    ...
    # so it's indexed by vm ems_ref. Then we try to fetch it with lazy_find_image_secs, which uses a by_uid_ems_and_name.
    ...
    lazy_find_image_secs = persister.miq_templates.lazy_find(
    ...
    @hardware_data_1 = hardware_data(1).merge(
      ...
      :guest_os_full_name => lazy_find_image_secs,
