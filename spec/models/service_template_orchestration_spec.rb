describe ServiceTemplateOrchestration do
  ...
  describe "#orchestration_manager" do
    let(:ems_alibaba) { FactoryBot.create(:ems_alibaba) }
    ...
    it "replaces the existing orchestration manager" do
      subject.orchestration_manager = ems_openstack
      subject.orchestration_manager = ems_alibaba

      expect(subject.orchestration_manager).to eq(ems_alibaba)
  ...
  describe "#my_zone" do
    context "with orchestration manager" do
      let(:ems_alibaba) { FactoryBot.create(:ems_alibaba) }
      before { subject.orchestration_manager = ems_alibaba }

      it "takes the zone from orchestration manager" do
        expect(subject.my_zone).to eq(ems_alibaba.my_zone)
      end
    end
