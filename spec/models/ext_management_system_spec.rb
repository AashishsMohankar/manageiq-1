describe ExtManagementSystem do
  ...
  let(:all_types_and_descriptions) do
    {
      ...
      "ecs"                         => "Alibaba ECS",
      "ecs_network"                 => "Alibaba ECS Network",
      "ecs_ebs_storage"             => "Alibaba EBS",
  ...
  context "#pause!" do
    ...
    it 'disables an ems and moves child managers created by ensure_managers() to maintenance zone' do
      zone  = FactoryGirl.create(:zone)
      emses = {
        :alibaba_cloud    => FactoryGirl.create(:ems_alibaba,          :zone => zone),
      ...
      # The same for storage managers, i.e. alibaba
      %i(alibaba_cloud openstack_cloud).each do |manager_type|
  ...
  describe ".raw_connect?" do
    it "returns true if validation was successful" do
      connection = double
      allow(ManageIQ::Providers::Alibaba::CloudManager).to receive(:raw_connect).and_return(connection)

      expect(ManageIQ::Providers::Alibaba::CloudManager.raw_connect?).to eq(true)
    end
  end
