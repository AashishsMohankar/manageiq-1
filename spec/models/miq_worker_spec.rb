describe MiqWorker do
  ...
  describe ".worker_settings" do
    ...
    context "at a concrete subclass" do
      let(:actual) { ManageIQ::Providers::Alibaba::CloudManager::RefreshWorker.worker_settings[:memory_threshold] }
      ...
      it "without overrides" do
        settings.store_path(:workers, :worker_base, :queue_worker_base, :ems_refresh_worker, :ems_refresh_worker_alibaba, {})
        stub_settings(settings)
    ...
    it "uses passed in config" do
      settings.store_path(:workers, :worker_base, :queue_worker_base, :ems_refresh_worker,
                          :ems_refresh_worker_alibaba, :memory_threshold, "5.terabyte")
      stub_settings(settings)

      settings.store_path(:workers, :worker_base, :queue_worker_base, :ems_refresh_worker,
                          :ems_refresh_worker_alibaba, :memory_threshold, "1.terabyte")
      actual = ManageIQ::Providers::Alibaba::CloudManager::RefreshWorker
  ...
  context "with two servers" do
    before do
      ...
      @worker = FactoryBot.create(:ems_refresh_worker_alibaba, :miq_server => @server)

      @server2 = FactoryBot.create(:miq_server, :zone => @zone)
      @worker2 = FactoryBot.create(:ems_refresh_worker_alibaba, :miq_server => @server2)
    end
    ...
    describe "#worker_settings" do
      ...
      it "without overrides" do
        @server.settings_changes.where(
          :key => "/workers/worker_base/queue_worker_base/ems_refresh_worker/ems_refresh_worker_alibaba/memory_threshold"
        ).delete_all
  ...
  describe ".config_settings_path" do
    ...
    it "include parent entries" do
      expect(capu_worker.config_settings_path).to eq(
        %i(workers worker_base queue_worker_base ems_metrics_collector_worker ems_metrics_collector_worker_alibaba)
      )
    end
