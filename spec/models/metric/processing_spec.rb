describe Metric::Processing do
  ...
  context ".process_derived_columns" do
    context "services" do
      let(:vm_alibaba_a) { FactoryBot.create(:vm_alibaba) }
      let(:vm_alibaba_b) { FactoryBot.create(:vm_alibaba, :powered_off) }
      let(:service) { FactoryBot.create(:service) }
      let(:metric_a) { FactoryBot.create(:metric_rollup_vm_hr, :resource => vm_alibaba_a) }
      let(:metric_b) { FactoryBot.create(:metric_rollup_vm_hr, :resource => vm_alibaba_b) }

      before do
        service.add_resource(vm_alibaba_a)
        service.add_resource(vm_alibaba_b)
        service.save
      end
